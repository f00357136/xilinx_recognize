...
module i2c_ov5640_rgb565_cfg
  #(
   ...
   (  
    ...
    input                i2c_done ,   //I2C寄存器配置完成信�?
    output  reg          i2c_exec ,   //I2C触发执行信号   
    output  reg  [23:0]  i2c_data ,   //I2C要配置的地址与数�?(�?16位地�?,�?8位数�?)
    output  reg          init_done    //初始化完成信�?
    );

//parameter define
localparam  REG_NUM = 8'd248  ;       //总共�?要配置的寄存器个�?
localparam  TOTAL_H_PIXEL = CMOS_H_PIXEL + 13'd1216;  //水平总像素大�?
localparam  TOTAL_V_PIXEL = CMOS_V_PIXEL + 13'd504;   //垂直总像素大�?

//reg define
reg   [14:0]   start_init_cnt;        //等待延时计数�?
...
//cam_scl配置�?250khz,输入的clk�?1Mhz,周期�?1us,20000*1us = 20ms
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        ...
        case(init_reg_cnt)
            ...
            8'd38 : i2c_data <= {16'h3c06,8'h00}; //light meter 1 阈�?�[15:8]
            8'd39 : i2c_data <= {16'h3c07,8'h08}; //light meter 1 阈�?�[7:0]
            8'd40 : i2c_data <= {16'h3c08,8'h00}; //light meter 2 阈�?�[15:8]
            8'd41 : i2c_data <= {16'h3c09,8'h1c}; //light meter 2 阈�?�[7:0]
            ...
            8'd48 : i2c_data <= {16'h4001,8'h02}; //BLC(黑电平校�?)补偿起始行号
            8'd49 : i2c_data <= {16'h4005,8'h1a}; //BLC(黑电平校�?)补偿始终更新
            8'd50 : i2c_data <= {16'h3000,8'h00}; //系统块复位控�?
            ...
            8'd242: i2c_data <= {16'h3b07,8'h0a}; //帧曝光模�?  
            //彩条测试使能 
            8'd243: i2c_data <= {16'h503d,8'h00}; //8'h00:正常模式 8'h80:彩条显示
            //测试闪光灯功�?
            8'd244: i2c_data <= {16'h3016,8'h02};
            8'd245: i2c_data <= {16'h301c,8'h02};
            8'd246: i2c_data <= {16'h3019,8'h02}; //打开闪光�?
            8'd247: i2c_data <= {16'h3019,8'h00}; //关闭闪光�?
            //只读存储�?,防止在case中没有列举的情况，之前的寄存器被重复改写
            default : i2c_data <= {16'h300a,8'h00}; //器件ID�?8�?
        endcase
    end
end
